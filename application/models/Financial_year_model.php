...
class Financial_year_model extends CI_Model
{
    ...
    function get_financial_year($id){
		...
		if(!(array)$result){
			$fields = $this->db->list_fields('financial_year');
	...
	function get_count_financial_year(){
	 ...
	 /** Get all users-financial_year
	 *
     */
    function get_all_users_financial_year(){
        $this->db->order_by('id', 'desc');
	...
	/** Get limit users-financial_year
	 *@param $limit - limit of query , $start - start of db table index to get query
	 *
     */
    function get_limit_users_financial_year($limit, $start){
		$this->db->order_by('id', 'desc');
        $this->db->limit($limit, $start);
		$this->db->where('users_id', $this->session->userdata('id'));
        $result = $this->db->get('financial_year')->result_array();
    ...
    /** Count users-financial_year rows
	 *
     */
	function get_count_users_financial_year(){
	   $this->db->where('users_id', $this->session->userdata('id'));
       $result = $this->db->from("financial_year")->count_all_results();
	   $db_error = $this->db->error();
		if (!empty($db_error['code'])){
