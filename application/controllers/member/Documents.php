...
class Documents extends CI_Controller{
	 ...
	 *@param $start - Starting of documents table's index to get query
	 *
	 */
    function index($start=0){
		...
		//pagination
		$config['base_url'] = site_url('member/documents/index');
        ...
        $data['_view'] = 'member/documents/index';
...
'updated_at' =>$updated_at,
							  ...
							  if(strlen($_FILES['file_picture']['name'])>0 && $_FILES['file_picture']['size']>0)
								{
									...
									{
										$file_picture = "uploads/images/documents/".$new_name;
        ...
        if(isset($id) && $id>0){
            ...
            if(isset($_POST) && count($_POST) > 0){   
                ...
                redirect('member/documents/index');
            }else{
                $data['_view'] = 'member/documents/form';
                $this->load->view('layouts/member/body',$data);
            }
        } //save
		else{
	...
	function details($id){
        $data['documents'] = $this->Documents_model->get_documents($id);
		$data['id'] = $id;
        $data['_view'] = 'member/documents/details';
        $this->load->view('layouts/member/body',$data);
	}
...
$this->db->or_like('updated_at', $key, 'both');
		...
		//pagination
		$config['base_url'] = site_url('member/documents/search');
		$this->db->reset_query();		
		$this->db->like('id', $key, 'both');
$this->db->or_like('innovation_plan_id', $key, 'both');
$this->db->or_like('document_file_type', $key, 'both');
...
$this->db->or_like('updated_at', $key, 'both');
	...
	function export($export_type='CSV'){
	  if($export_type=='CSV'){	
		   ...
		   $header = array("Id","Innovation Plan Id","Document File Type","File Picture","Description","Created At","Updated At"); 
		   fputcsv($file, $header);
		   ...
		   // get the HTML
			ob_start();
			include(APPPATH.'views/member/documents/print_template.php');
			$html = ob_get_clean();
