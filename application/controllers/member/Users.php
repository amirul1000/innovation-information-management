...
class Users extends CI_Controller{
    ...
    function index($start=0){
		$limit = 10;
        $data['users'] = $this->Users_model->get_limit_users_users($limit,$start);
		//pagination
		$config['base_url'] = site_url('member/users/index');
		$config['total_rows'] = $this->Users_model->get_count_users_users();
		$config['per_page'] = 10;
    ...
    function save($id=-1){   
		...
		$params = array(
						'owner_users_id' => $this->session->userdata('id'),
							  ...
							  if(strlen($_FILES['file_picture']['name'])>0 && $_FILES['file_picture']['size']>0)
								{
									...
									{
										$file_picture = "uploads/images/users/".$new_name ;
        ...
        if(isset($id) && $id>0){
            ...
            if(isset($_POST) && count($_POST) > 0){   
                ...
                redirect('member/users/index');
            }else{
                $data['_view'] = 'member/users/form';
                $this->load->view('layouts/member/body',$data);
            }
        } //save
		else{
	...
	function details($id){
        $data['users'] = $this->Users_model->get_users($id);
		$data['id'] = $id;
        $data['_view'] = 'member/users/details';
        $this->load->view('layouts/member/body',$data);
	}
	...
	function search($start=0){
		...
		//pagination
		$config['base_url'] = site_url('member/users/search');
		$this->db->reset_query();		
		if( $this->session->userdata('evaluator')==true){
			$this->db->where('owner_users_id', $this->session->userdata('id'));
		}
	...
	function export($export_type='CSV'){
	  if($export_type=='CSV'){	
		   ...
		   // get data 
		   $this->db->order_by('id', 'desc');
		   $usersData = $this->Users_model->get_all_users_users();
		   // file creation 
		   $file = fopen('php://output', 'w');
		   $header = array("Id","Department Id","Email","Password","Title","First Name","Last Name","File Picture","Phone No","Dob","Company","Address","City","State","Zip","Country Id","Created At","Updated At","User Type","Status"); 
		   fputcsv($file, $header);
		   ...
		   exit; 
		    ...
		    $users = $this->Users_model->get_all_users_users();;
		   // get the HTML
			ob_start();
			include(APPPATH.'views/member/users/print_template.php');
			$html = ob_get_clean();
